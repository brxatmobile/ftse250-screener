...
# FILE_VERSION: INTRADAY_RETAIN_AFTER_10_2026_08_03
# PREVIOUS_FILE_VERSION: INTRADAY_DAYTRADER_NO_EMAIL_2026_08_02
...
<div id="expired-content" class="expired"><h2>Past the 10:00 cutoff</h2><p>The recommendations below are retained for review only. Do not place a trade from these figures now because market prices and the appropriate stop and target may have changed.</p><p><a href="index.html">Daily watchlist</a> · <a href="backtest.html">Backtest</a></p></div>
...
(function(){
  ...
  const expired=document.getElementById('expired-content');
  function enforce(){
    if(new Date()>=expiry){
      ...
      document.querySelectorAll('.recommendation').forEach(function(el){
        ...
        el.textContent='PAST 10:00 — Do not follow this recommendation now. Original assessment: '+original;
      });
    }
  }
  enforce();
...
def main() -> int:
    ...
    if mode == "expire":
        # Keep the most recent assessment visible. Its browser-side cutoff logic
        # relabels every recommendation as expired after 10:00 London time.
        if OUTPUT_PATH.exists():
            print(f"Past 10:00: retaining the existing intraday report at {OUTPUT_PATH}.")
        else:
            print("Past 10:00: no existing intraday report is available to retain.")
        return 0
